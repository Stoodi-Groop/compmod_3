...
#get priors
get_prior(data_f, data = data, family = gaussian()) #we need to set priors for b, intercept, sigma, and sd 
...
data_informed_prior_edit <- c(
  ...
  prior(normal(0,0.5), class = Intercept),
  prior(normal(0,0.3), class = sd), #change in prior from 0.1 to 0.3
  prior(normal(0, 1), class = sigma))
